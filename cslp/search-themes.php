...
<body>
	...
	$query = "SELECT * FROM `rso-themes` WHERE ";
	if(!trim($keyword)){
			header("Location: index.php");
	} elseif($keyword == "--all"){
			$query = mysqli_query($conn, $query . "1;");
	} else{
			$query = mysqli_query($conn, $query . "`rso-themes`.theme LIKE '%".$keyword."%' ORDER BY `rso-themes`.theme ASC;");	
	}
	if(mysqli_num_rows($query)>0){
		...
		while($row=mysqli_fetch_row($query)){
			echo "<tr><td>".$row[1]."</td><td align='center'><a href=view-themes.php?id=$row[0]>[VIEW]</a></td></tr>";
		}
		echo "</tbody></table>";
	}else{
