...
<body>
	...
	$id = $_GET['id'];
	$query = mysqli_query($conn, "SELECT `rso-areas`.id, `rso-areas`.areas, `rso-research-titles`.research_title, `rso-research-titles`.id FROM `rso-areas` LEFT JOIN `rso-research-titles` ON `rso-areas`.id=`rso-research-titles`.area_id WHERE `rso-research-titles`.area_id=".$id.";");
